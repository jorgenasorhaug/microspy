...
# Silence hyperspy's 'signal_type not understood' warning

import logging

class IgnoreSignalType(logging.Filter):
    def filter(self, record):
        return "not understood" not in record.getMessage()

logger = logging.getLogger("hyperspy.io")
logger.addFilter(IgnoreSignalType())
